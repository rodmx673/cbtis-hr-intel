...
import { Settings, Pencil, PlusCircle } from 'lucide-react';
...
import type { Nivel, DistribucionRow } from '@/lib/types';
...
export default function GestionHorariosAsignaturasBase() {
  ...
  useEffect(() => {
    if (isClient && niveles.length > 0) {
      if (!niveles.find(n => n.id === nivelSeleccionado)) {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="text-center">
          <h1 className="text-3xl font-bold tracking-tight text-foreground sm:text-4xl">
            HORARIOS INTELIGENTES: CBTIS 55
          </h1>
          ...
          {niveles.length > 0 && (
            <>
              ...
              <div className="flex items-center gap-2">
                <Label htmlFor="horas-objetivo" className="text-muted-foreground">
                  Meta de Horas:
                </Label>
                <Input
                  id="horas-objetivo"
        ...
        <Tabs defaultValue="carga-horaria" className="w-full">
          <TabsList className="grid w-full grid-cols-2 sm:grid-cols-4 md:grid-cols-7">
            <TabsTrigger value="carga-horaria">Carga Horaria</TabsTrigger>
            <TabsTrigger value="distribucion">Distribución por Grupo</TabsTrigger>
            <TabsTrigger value="docentes">Docentes</TabsTrigger>
            <TabsTrigger value="restricciones" asChild>
              <Link href="/restricciones">Restricciones</Link>
            </TabsTrigger>
            <TabsTrigger value="horarios-fijos" asChild>
              <Link href="/restricciones/horario-fijo">Horarios Fijos</Link>
            </TabsTrigger>
            <TabsTrigger value="horario-grupo" asChild>
              <Link href="/horario">Horario por Grupo</Link>
            </TabsTrigger>
            <TabsTrigger value="horario-docente" asChild>
              <Link href="/horario/docente">Horario por Docente</Link>
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="carga-horaria">
            {nivelSeleccionado && currentNivel ? (
              <Card className="shadow-md border-border mt-4">
                <CardHeader>
                  <CardTitle>Carga Horaria Base: {currentNivel.nombre}</CardTitle>
                  <CardDescription>
              ...
              <div className="text-center py-10">
                <p className="text-muted-foreground">
                  {niveles.length > 0
                    ? 'Seleccione un nivel para ver su configuración.'
                    : 'Agregue un nivel para comenzar.'}
                </p>
              </div>
            )}
          </TabsContent>
            ...
            {nivelSeleccionado && currentNivel ? (
              <Card className="shadow-md border-border mt-4">
                <CardHeader className="flex flex-row items-center justify-between">
                  <div>
                    <CardTitle>Distribución Académica por Grupo: {currentNivel.nombre}</CardTitle>
                    <CardDescription>
                  ...
                  <Button
                    ...
                    variant="outline"
                    disabled={
                      hasEmptyDistribucionRow ||
                      asignaturasDisponiblesDistribucion.length === 0 ||
                      !editMode
                    }
                  >
